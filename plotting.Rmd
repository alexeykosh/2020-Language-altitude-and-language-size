...
знают ли люди, которые живут выше, язык соседей, которые живут ниже? но не наоборот
- схлопунть все деревни, где говорят на одном языке в кластере, и посчитать сред. высотность

язык -- min(villages_with_this_language$elevation)
calulcate altitude of each L in each cluster as the minimum altitude of all villages where the L is spoken in the cluster
model predicts the knowledge of L2 based on the difference in the altitude between L1 of the village and L2

one datapoint -- one language 
...
### extract degree from bigger dataset !
